"""
Phase 3 - WebSocket Server: 打断 + 智能断句
集成 EchoJudge、InterruptDetector、SentenceJudge
"""
...
vad = VADEngine(
    ...
    min_speech_frames=4,
    min_silence_frames=12,  # ~480ms，SentenceJudge 在此基础上智能判断
)
            ...
            if "bytes" in data:
                ...
                if s in (LISTEN, IDLE):
                    ...
                    if event == "speech_end" and s == LISTEN:
                        ...
                        await _start_llm_tts(ws, judged_text, llm_cancel, tts_task, session)
                ...
                if session["state"] == LISTEN and sentence_judge.has_pending:
                    ...
                    if pending:
                        ...
                        await _start_llm_tts(ws, pending, llm_cancel, tts_task, session)
...
async def _start_llm_tts(ws, text, llm_cancel, tts_task, session):
    """启动 LLM 流式生成 + TTS 流式合成"""
...
<html>
<head><meta charset="utf-8"><title>Smart RangRang - Phase 3</title></head>
<body>
<h2>Phase 3 打断 + 智能断句测试</h2>
...
<div><b>断句:</b> <span id="sentence"></span></div>
